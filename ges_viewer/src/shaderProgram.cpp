...
void Program::SetUniformRaw(int loc, const glm::ivec2& value) const
{
    glUniform2iv(loc, 1, (int*)&value);
}
...
void Program::SetTex2D(std::string name, int tex, int index) const{
    glActiveTexture(GL_TEXTURE0 + index);
    glUniform1i(glGetUniformLocation(program, name.c_str()), index);
    glBindTexture(GL_TEXTURE_2D, tex);
}
void Program::SetTex2DMS(std::string name, int tex, int index) const{
    glActiveTexture(GL_TEXTURE0 + index);
    glUniform1i(glGetUniformLocation(program, name.c_str()), index);
