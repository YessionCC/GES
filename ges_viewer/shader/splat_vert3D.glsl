...
void main(void)
{
    ...
    float tan_fovx = 1.0 / projMat[0][0];
    float tan_fovy = 1.0 / projMat[1][1];
    ...
    float SX = projMat[0][0]; // SX = 1 / tan_half_fovx = fx / w/2 //  tan_half_fovx = w/2 / fx
    ...
    mat3 W = mat3(viewMat);
    mat3 V = mat3(splatRS_0, splatRS_1, splatRS_2);
    mat3 JW = J * W;
    ...
    vec4 p4 = projMat * viewMat * vec4(position.xyz, 1.0f);
    ...
    geom_color = vec4(ComputeRadianceFromSH(v), alpha * coef);
