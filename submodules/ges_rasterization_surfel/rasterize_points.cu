	...
	const bool debug)
{
  ...
  //torch::Tensor out_others = torch::full({3+1+1, H, W}, 0.0, float_opts);
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor cover_area = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		nullptr,//out_others.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		cover_area.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, radii, cover_area, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  nullptr, //dL_dout_others.contiguous().data<float>(),
