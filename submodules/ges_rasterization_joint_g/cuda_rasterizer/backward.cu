...
// each Gaussian.
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, int C, const glm::vec3* means, glm::vec3 campos, const float* shs, const float* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	...
	glm::vec4* dL_drot)
{
	...
	if (shs)
		computeColorFromSH(idx, D, M, C, (glm::vec3*)means, *campos, shs, dL_dcolor, (glm::vec3*)dL_dmeans, (glm::vec3*)dL_dsh);
...
renderCUDA(
	...
	const float* __restrict__ center_depths, //
	const float* __restrict__ weightsum_map, //
	const float* __restrict__ colors,
	const float* __restrict__ dL_dpixels,
	const float* __restrict__ dL_dothers, //
	float* __restrict__ max_contrib, //
	...
	float* __restrict__ dL_dcolors)
{
	...
	__shared__ float collected_c_depths[BLOCK_SIZE];
	...
	if (inside) {
		...
		dL_dweightsum = dL_dothers[pix_id];
		pix_depth = depth_map[pix_id];
		pix_ws = weightsum_map[pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_c_depths[block.thread_rank()] = center_depths[coll_id];
		...
		{
			float depth = collected_c_depths[j];
			if(depth > pix_depth) continue;
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				if (ch < 3)
					max_abs_c = max(max_abs_c, abs(c));
				dL_dalpha += c * dL_dchannel;
				atomicAdd(&(dL_dcolors[global_id * C + ch]), alpha * dL_dchannel);
			}
			dL_dalpha += dL_dweightsum;

			atomicMaxFloat(&max_contrib[global_id], max_abs_c * alpha / (pix_ws + 1.0f));
