...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped, const bool need_clamp)
{
	...
	const bool need_clamp)
{
	...
	float cutoff = sqrtf(max(11.1f + 2.f * logf(opacities[idx]), 0.000001));
#else
	float cutoff = 3.4f;
#endif
	...
	if (colors_precomp == nullptr) {
		glm::vec3 result = computeColorFromSH(idx, D, M, (glm::vec3*)orig_points, *cam_pos, shs, clamped, need_clamp);
	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, _toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, _toDo); j++)
		{
	...
	// computing the minimal depth gaussian
	if(closeGeoIdx > 0) {
		atomicAdd(&cover_area[closeGeoGlobalIdx], 1); //

		float test_T = T * (1 - closeGeoAlpha);
		float w = closeGeoAlpha * T;
#if RENDER_AXUTILITY
		// Render depth and normal map
		D  += minDepth * w;
		float4 nor_o = normal_opacity[closeGeoGlobalIdx];
		N[0] += nor_o.x * w; N[1] += nor_o.y * w; N[2] += nor_o.z * w;
#endif
		// Eq. (3) from 3D Gaussian splatting paper.
		for (int ch = 0; ch < CHANNELS; ch++)
			C[ch] += features[closeGeoGlobalIdx * CHANNELS + ch] * w;
		T = test_T;
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if(contributor == closeGeoIdx) continue;
	...
	if (inside)
	{
		...
		n_contrib[pix_id + H*W] = closeGeoIdx;
		for (int ch = 0; ch < CHANNELS; ch++)
