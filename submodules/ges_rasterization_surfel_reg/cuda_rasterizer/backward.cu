...
// each Gaussian.
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, bool need_means_grad, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	...
	const bool use_filter)
{
	...
	const int minGeoIdx = inside ? (int)n_contrib[pix_id + H*W] - 1 : -1;
	float minGeoAlpha = 0.0;
	float minGeoDepth = 0.0;
	float minGeoG = 0.0;
	bool minGeoRho3d = false;
	int minGeoGlobalIdx = 0;
	float3 minGeok;float3 minGeop;float3 minGeol;float2 minGeod;float2 minGeos;float3 minGeoTw;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			contributor--;
			// in case when minGeoIdx > last_contributor

			
			if (contributor >= last_contributor && contributor != minGeoIdx)
				continue;
			...
			float depth, rho, rho2d; float2 d;
			if (use_filter) {
			...
			if (alpha < 1.0f / 255.0f || G < 1.0f / 255.0f)
				continue;

			if (contributor == minGeoIdx) {
				minGeoAlpha = alpha;
				minGeoDepth = depth;
				minGeoG = G;
				minGeoRho3d = rho3d <= rho2d;
				minGeok = k; minGeop = p; minGeol = l; 
				minGeod = d; minGeos = s; minGeoTw = Tw;
				minGeoGlobalIdx = collected_id[j];
				continue;
			}
		...
		for (int ch = 0; ch < 3; ch++) {
			...
			const float dL_dG = nor_o.w * G < 1.0f ?
					nor_o.w * dL_dalpha : 0.0f;

			if (!use_filter || rho3d <= rho2d) {
	...
	if(minGeoIdx >= 0) {
		T = T / (1.f - minGeoAlpha);
		const float dchannel_dcolor = minGeoAlpha * T;
		const float w = minGeoAlpha * T;
		// Propagate gradients to per-Gaussian colors and keep
		// gradients w.r.t. alpha (blending factor for a Gaussian/pixel
		// pair).
		float dL_dalpha = 0.0f;
		const int global_id = minGeoGlobalIdx;
		for (int ch = 0; ch < C; ch++)
		{
	...
	// Propagate gradients w.r.t ray-splat depths
	accum_depth_rec = last_alpha * last_depth + (1.f - last_alpha) * accum_depth_rec;
	last_depth = minGeoDepth;
	dL_dalpha += (minGeoDepth - accum_depth_rec) * dL_ddepth;
	// Propagate gradients w.r.t. color ray-splat alphas
	accum_alpha_rec = last_alpha * 1.0 + (1.f - last_alpha) * accum_alpha_rec;
	dL_dalpha += (1 - accum_alpha_rec) * dL_daccum;

	// Propagate gradients to per-Gaussian normals
	float4 nor_o = normal_opacity[minGeoGlobalIdx];
	float normal[3] = {nor_o.x, nor_o.y, nor_o.z};
	for (int ch = 0; ch < 3; ch++) {
		accum_normal_rec[ch] = last_alpha * last_normal[ch] + (1.f - last_alpha) * accum_normal_rec[ch];
		last_normal[ch] = normal[ch];
		dL_dalpha += (normal[ch] - accum_normal_rec[ch]) * dL_dnormal2D[ch];
		atomicAdd((&dL_dnormal3D[global_id * 3 + ch]), minGeoAlpha * T * dL_dnormal2D[ch]);
	}
#endif

		dL_dalpha *= T;
		// Update last alpha (to be used in the next iteration)
		last_alpha = minGeoAlpha;
		...
		for (int i = 0; i < C; i++)
			bg_dot_dpixel += bg_color[i] * dL_dpixel[i];
		dL_dalpha += (-T_final / (1.f - minGeoAlpha)) * bg_dot_dpixel;

#if RENDER_AXUTILITY
		dL_dz += minGeoAlpha * T * dL_ddepth; 
#endif		

		const float dL_dG = nor_o.w * minGeoG < 1.0f ?
				nor_o.w * dL_dalpha : 0.0f;

		if (!use_filter || minGeoRho3d) {
			// Update gradients w.r.t. covariance of Gaussian 3x3 (T)
			const float2 dL_ds = {
				dL_dG * -minGeoG * minGeos.x + dL_dz * minGeoTw.x,
				dL_dG * -minGeoG * minGeos.y + dL_dz * minGeoTw.y
			};
			const float3 dz_dTw = {minGeos.x, minGeos.y, 1.0};
			const float dsx_pz = dL_ds.x / minGeop.z;
			const float dsy_pz = dL_ds.y / minGeop.z;
			const float3 dL_dp = {dsx_pz, dsy_pz, -(dsx_pz * minGeos.x + dsy_pz * minGeos.y)};
			const float3 dL_dk = cross(minGeol, dL_dp);
			const float3 dL_dl = cross(dL_dp, minGeok);
			...
			// // Update gradients w.r.t. center of Gaussian 2D mean position
			const float dG_ddelx = -minGeoG * FilterInvSquare * minGeod.x;
			const float dG_ddely = -minGeoG * FilterInvSquare * minGeod.y;
			atomicAdd(&dL_dmean2D[global_id].x, dL_dG * dG_ddelx); // not scaled
			atomicAdd(&dL_dmean2D[global_id].y, dL_dG * dG_ddely); // not scaled
			atomicAdd(&dL_dtransMat[global_id * 9 + 8],  dL_dz); // propagate depth loss
		}

		if (nor_o.w * minGeoG < 1.0f) {
			// Update gradients w.r.t. opacity of the Gaussian
			atomicAdd(&(dL_dopacity[global_id]), minGeoG * dL_dalpha);
		}
	}
}
...
__global__ void preprocessCUDA(
	...
	const float disable_sh_means_grad_opac_thr,
	...
	glm::vec4* dL_drots)
{
	...
	if (shs) {
		const float opacity = normal_opacity[idx].w;
		bool need_means_grad = opacity < disable_sh_means_grad_opac_thr; // if opacity is greater than the thr, sh_means_grad is disabled
		
		computeColorFromSH(idx, D, M, need_means_grad, (glm::vec3*)means3D, *campos, shs, clamped, (glm::vec3*)dL_dcolors, (glm::vec3*)dL_dmean3Ds, (glm::vec3*)dL_dshs);
	}
...
{	
	preprocessCUDA<NUM_CHANNELS><< <(P + 255) / 256, 256 >> > (
		...
		(glm::vec4*)rotations,
		normal_opacity,
		disable_sh_means_grad_opac_thr,
		scale_modifier,
		viewmatrix,
		projmatrix,
		focal_x, 
