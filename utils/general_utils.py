...
# mat: x,2,2, sym
def decompose_RS(mat: torch.Tensor):
    A = mat[:,0,0]
    B = mat[:,0,1]*2
    C = mat[:,1,1]
    p = A - C
    q = A + C
    t = torch.sign(p) * torch.sqrt(p**2 + B**2)
    S = torch.zeros_like(mat)
    S[:,0,0] = torch.sqrt((q+t)/2)
    S[:,1,1] = torch.sqrt((q-t)/2)
    R = torch.zeros_like(mat)
    R00 = torch.where(t!=0, torch.sqrt((t+p)/(2*t)), torch.ones_like(t))
    R10 = torch.where(t!=0, torch.sign(B*p)*torch.sqrt((t-p)/(2*t)), torch.zeros_like(t))
    R[:,0,0] = R00; R[:,1,1] = R00
    R[:,1,0] = R10; R[:,0,1] = -R10
    return R, S